...
var pausedSimulation = config.PAUSED;
...
let ditheringTexture = createTextureAsync("img/dithering_texture.png");
...
if (config.SPLATSONLOAD) {
    multipleSplats(parseInt(Math.random() * config.SPLATSMAX) + config.SPLATSMIN);
}
...
canvas.addEventListener("mousedown", (e) => {
    if (!config.SPLATSONMOUSEDOWN) return;

    splatStack.push(
        parseInt(Math.random() * config.SPLATSMAX) + config.SPLATSMIN
    );
});
...
canvas.addEventListener("mousemove", (e) => {
    ...
    timeout = setTimeout(mouseStop, 50, e);
});
